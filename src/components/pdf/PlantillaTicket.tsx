...
export interface TicketData {
  ...
  comprobante: {
    ...
    tipo: 'boleta' | 'factura' | 'nota_credito' | 'nota_venta' | 'nota_venta_interna' | 'proforma'
    fecha: string
...
export default function PlantillaTicket({ data }: { data: TicketData }) {
  ...
  const isNotaVenta = data.comprobante.tipo === 'nota_venta' || data.comprobante.tipo === 'nota_venta_interna' || !data.comprobante.tipo
    ...
    <Document>
      <Page style={styles.page}>
        <View style={styles.header}>
          ...
          {data.ferreteria.ruc && !isNotaVenta && (
        ...
        <View style={{ alignItems: 'center', marginBottom: 4 }}>
          ...
          <Text style={{ fontSize: 10, marginTop: 2 }}>{data.comprobante.numero_completo}</Text>
        </View>

        <View style={styles.separator} />

        <View style={{ marginBottom: 4, gap: 2 }}>
          <Text><Text style={{ fontWeight: 'bold' }}>Fecha:</Text> {format(new Date(data.comprobante.fecha), 'dd/MM/yyyy HH:mm')}</Text>
          <Text><Text style={{ fontWeight: 'bold' }}>Cliente:</Text> {data.comprobante.cliente_nombre}</Text>
          {data.comprobante.cliente_doc && (
            <Text><Text style={{ fontWeight: 'bold' }}>{isFactura ? 'RUC' : 'DNI/Doc'}:</Text> {data.comprobante.cliente_doc}</Text>
          )}
        </View>
        ...
        <View style={styles.totalSection}>
          ...
          <View style={[styles.totalRow, { marginTop: isNotaVenta ? 2 : 0 }]}>
            <Text style={[styles.totalLabel, { fontSize: 10 }]}>TOTAL A PAGAR</Text>
            <Text style={[styles.totalValue, { fontSize: 10 }]}>S/ {data.comprobante.total.toFixed(2)}</Text>
          </View>
        </View>
        ...
        <View style={styles.footer}>
          {isNotaVenta ? (
            <>
              <Text style={{ fontWeight: 'bold', fontSize: 10, marginBottom: 4, marginTop: 4 }}>¡GRACIAS POR SU COMPRA!</Text>
              <Text style={[styles.legalText, { fontSize: 8, color: '#555' }]}>Documento de control interno sin validez tributaria.</Text>
            </>
          ) : (
            <>
