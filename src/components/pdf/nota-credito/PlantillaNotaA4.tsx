...
import { PropsNota } from './types'

export default function PlantillaNotaA4({ emisor, comprobante, items, tema, tipoNota, documentoAfectado, motivoDescripcion }: PropsNota) {
  registerFonts()

  const titulo = tipoNota === 'debito' ? 'NOTA DE DÉBITO ELECTRÓNICA' : 'NOTA DE CRÉDITO ELECTRÓNICA'
  ...
  const styles = StyleSheet.create({
    ...
    docType: { fontSize: 13, fontWeight: 'bold', marginVertical: 6, color: tema.primario, textAlign: 'center' },
    docNum: { fontSize: 12 },

    modificaBox: { backgroundColor: '#fef2f2', borderWidth: 1, borderColor: '#fecaca', borderRadius: 4, padding: 10, marginBottom: 16 },
    modificaLabel: { fontSize: 8, color: '#991b1b', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 3 },
    modificaText: { fontSize: 10, fontWeight: 'bold', color: '#7f1d1d' },
    motivoText: { fontSize: 9, color: '#7f1d1d', marginTop: 3 },
      ...
      <Page size="A4" style={styles.page}>
        <View style={styles.header}>
          ...
          <View style={styles.docBox}>
            ...
            <Text style={styles.docType}>{titulo}</Text>
            <Text style={styles.docNum}>{comprobante.numero_completo}</Text>
          </View>
        </View>

        <View style={styles.modificaBox}>
          <Text style={styles.modificaLabel}>Documento que modifica</Text>
          <Text style={styles.modificaText}>{documentoAfectado}</Text>
          {motivoDescripcion && <Text style={styles.motivoText}>Motivo: {motivoDescripcion}</Text>}
        </View>
        ...
        <View style={styles.footer}>
          ...
          <View style={styles.legalBox}>
            <Text style={styles.legalText}>Representación impresa de la {titulo.toLowerCase()}.</Text>
