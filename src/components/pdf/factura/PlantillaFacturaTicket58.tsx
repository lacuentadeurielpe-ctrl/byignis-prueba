...
export default function PlantillaFacturaTicket58({ emisor, comprobante, items, tema }: PropsFactura) {
  registerFonts()
  ...
  const styles = StyleSheet.create({
    ...
    title: { fontSize: 6, fontWeight: 'bold', marginBottom: 2, textAlign: 'center', color: tema.primario },
    ...
    totalLabel: { fontWeight: 'bold', fontSize: 6 },
    totalValue: { fontWeight: 'bold', fontSize: 6 },
    
    footer: { marginTop: 8, alignItems: 'center' },
    footerTitle: { fontWeight: 'bold', fontSize: 6, marginBottom: 4, marginTop: 4, color: tema.primario },
      ...
      <Page style={styles.page}>
        ...
        {items.map((item, i) => (
          ...
          <View style={styles.totalRow}>
            <Text style={{ fontSize: 7 }}>Op. Gravada</Text>
            <Text style={{ fontSize: 7 }}>{formatMonto(comprobante.subtotal)}</Text>
          </View>
          <View style={styles.totalRow}>
            <Text style={{ fontSize: 7 }}>IGV (18%)</Text>
            <Text style={{ fontSize: 7 }}>{formatMonto(comprobante.igv)}</Text>
          </View>
