  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <Link href="/dashboard/ventas?tab=pedidos" className="lg:col-span-2 bg-white/80 backdrop-blur-sm rounded-2xl border border-zinc-100 p-5 hover:border-zinc-200 hover:shadow-sm transition block group">
          <h3 className="text-sm font-semibold text-zinc-900 mb-4 group-hover:text-blue-600 transition-colors">Tendencia de actividad</h3>
          <ActivityChart datos={chartData} />
        </Link>

        <div className="bg-white/80 backdrop-blur-sm rounded-2xl border border-zinc-100 p-5 hover:shadow-sm transition">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-semibold text-zinc-900">Más cotizados (30d)</h3>
            <Link href="/dashboard/catalog" className="text-xs text-zinc-400 hover:text-zinc-900 transition font-medium flex items-center gap-0.5 group">
                ...
                return (
                  <Link key={prod.nombre} href="/dashboard/catalog" className="block group hover:opacity-80 transition">
                    <div className="flex items-center justify-between mb-1.5">
                      ...
                      <span className="text-xs font-bold text-zinc-700 shrink-0 tabular-nums">{prod.cantidad}</span>
                    </div>
                    <div className="w-full bg-zinc-100 rounded-full h-1.5 overflow-hidden">
                      <motion.div 
                        ...
                        className="bg-gradient-to-r from-zinc-800 to-zinc-600 h-1.5 rounded-full" 
                      />
                    </div>
                  </Link>
                )
