...
export default function DashboardSnapshot({ esDueno }: { esDueno: boolean }) {
  ...
  return (
    <div className="bg-zinc-950 rounded-2xl p-5 text-white overflow-hidden relative shadow-lg">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 relative z-10">
        <Link href="/dashboard/ventas?tab=pagos" className="group p-3 rounded-xl bg-white/[0.04] hover:bg-white/[0.08] transition cursor-pointer backdrop-blur-sm border border-white/5">
          <p className="text-2xl sm:text-3xl font-bold tabular-nums">
            {esDueno ? formatPEN(ingresosHoy) : String(pedidosHoyN)}
          </p>
        ...
        <Link href="/dashboard/ventas?tab=pedidos" className="group p-3 rounded-xl bg-white/[0.04] hover:bg-white/[0.08] transition cursor-pointer backdrop-blur-sm border border-white/5">
        ...
        <Link href="/dashboard/ventas?tab=pedidos" className="group p-3 rounded-xl bg-white/[0.04] hover:bg-white/[0.08] transition cursor-pointer backdrop-blur-sm border border-white/5">
          <p className={cn('text-2xl sm:text-3xl font-bold tabular-nums', cobrosN > 0 ? 'text-amber-400 drop-shadow-[0_0_8px_rgba(251,191,36,0.3)]' : '')}>{cobrosN}</p>
          <p className="text-xs text-zinc-500 mt-1">por cobrar hoy</p>
        </Link>
        
        <Link href="/dashboard/conversations" className="group p-3 rounded-xl bg-white/[0.04] hover:bg-white/[0.08] transition cursor-pointer backdrop-blur-sm border border-white/5">
          <p className={cn('text-2xl sm:text-3xl font-bold tabular-nums', convActivas > 0 ? 'text-sky-400 drop-shadow-[0_0_8px_rgba(56,189,248,0.3)]' : '')}>{convActivas}</p>
