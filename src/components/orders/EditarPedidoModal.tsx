...
import { X, Plus, Trash2, Search, Loader2, Package, Check, Pencil } from 'lucide-react'
...
export default function EditarPedidoModal({ pedido, productos, zonas, onClose }: EditarPedidoModalProps) {
  const router = useRouter()

  // Inicializar items desde el pedido existente
  const [items, setItems] = useState<ItemCarrito[]>(() =>
    pedido.items_pedido.map((item) => {
      const prod = productos.find((p) => p.nombre === item.nombre_producto)
      return {
  ...
  const [nombreCliente, setNombreCliente] = useState(
    pedido.clientes?.nombre ?? pedido.nombre_cliente ?? ''
  )
  const [telefonoCliente, setTelefonoCliente] = useState(
    pedido.clientes?.telefono ?? pedido.telefono_cliente ?? ''
  )
  const [modalidad, setModalidad] = useState<'delivery' | 'recojo'>(
    (pedido.modalidad as 'delivery' | 'recojo') ?? 'recojo'
  )
  ...
  // Memoizado con debounce — evita Levenshtein en cada tecla (BUG-007)
  ...
  function agregarProducto(p: Producto) {
    ...
    if (existe >= 0) {
      setItems((prev) =>
        prev.map((i, idx) => (idx === existe ? { ...i, cantidad: i.cantidad + 1 } : i))
      )
    } else {
      setItems((prev) => [
        ...prev,
    ...
    try {
      const res = await fetch(`/api/orders/${pedido.id}`, {
        ...
        body: JSON.stringify({
          ...
          modalidad,
          direccion_entrega: modalidad === 'delivery' ? direccion.trim() : null,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div>
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-base font-bold text-gray-900">Editar pedido</h2>
            </div>
            <p className="text-xs text-gray-400 mt-0.5 font-mono">{pedido.numero_pedido}</p>
          </div>
        ...
        {/* Aviso importante */}
        <div className="mx-6 mt-4 px-4 py-2.5 bg-amber-50 border border-amber-200 rounded-xl">
          <p className="text-xs text-amber-700">
            <span className="font-semibold">⚠️ Solo puedes editar pedidos sin comprobante emitido a SUNAT.</span>
            {' '}Los cambios actualizarán los productos, datos del cliente y modalidad de entrega.
          </p>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-6">
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              ...
              <button
                ...
                className="text-xs text-amber-600 hover:text-amber-700 transition"
              >
              ...
              <div className="relative" data-busqueda>
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-3 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-amber-400 transition"
                  />
                </div>
              ...
              <div className="bg-amber-50 rounded-xl p-4 space-y-3">
                <div className="grid grid-cols-2 gap-3">
                  <div className="col-span-2">
                    <input
                      ...
                      className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-amber-400 transition bg-white"
                    />
                  </div>
                  <input
                    ...
                    className="px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-amber-400 transition bg-white"
                  />
                  <input
                    type="number"
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    min={1}
                    className="w-20 px-3 py-2 border border-gray-200 rounded-lg text-sm text-center focus:outline-none focus:ring-2 focus:ring-amber-400 transition bg-white"
                  />
                  <span className="text-sm text-gray-500 flex-1">unidades</span>
                  <button
                    type="button"
                    onClick={agregarManual}
                    className="flex items-center gap-1.5 px-3 py-2 bg-amber-500 text-white rounded-lg text-sm hover:bg-amber-600 transition"
                  >
                  ...
                  <div key={idx} className="flex items-center gap-2 bg-gray-50 rounded-lg px-3 py-2">
                    ...
                    <div className="flex items-center gap-1.5 shrink-0">
                      <input
                        ...
                        min={1}
                        className="w-14 px-2 py-1 border border-gray-200 rounded text-sm text-gray-900 text-center bg-white focus:outline-none focus:ring-1 focus:ring-amber-400"
                      />
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          min={0}
                          step={0.01}
                          className="w-20 pl-6 pr-2 py-1 border border-gray-200 rounded text-sm text-gray-900 bg-white focus:outline-none focus:ring-1 focus:ring-amber-400"
                        />
                      </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-amber-400 transition"
                />
              </div>
              <div>
                ...
                {zonas.length > 0 && (
                  <div>
                    ...
                    <select
                      ...
                      className="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-amber-400 transition bg-white"
                    >
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-amber-400 transition resize-none"
            />
          </div>
        </div>
        ...
        <div className="px-6 py-4 border-t border-gray-100 flex items-center gap-3">
          ...
          <button
            ...
            onClick={guardar}
            disabled={guardando}
            className="flex items-center gap-2 px-5 py-2.5 bg-amber-500 hover:bg-amber-600 disabled:opacity-60 text-white font-medium rounded-lg text-sm transition"
          >
            {guardando
